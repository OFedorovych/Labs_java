...
public class GoodOrderController {
    ...
    public ResponseEntity<String> deleteGood(@PathVariable(name = "id") Integer id) {
        try {
            return new ResponseEntity<String>(goodOrderService.delGood(id), HttpStatus.OK);
        } catch (NoSuchElementException e) {
            LOGGER.severe("Can't delete an order with non-existing id " + id);
